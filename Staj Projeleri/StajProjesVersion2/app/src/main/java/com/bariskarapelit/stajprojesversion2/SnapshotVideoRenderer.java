...
public class SnapshotVideoRenderer implements VideoRenderer {
    ...
    public void renderFrame(final I420Frame i420Frame) {
        // Bitmap'i yakalayın ve ana ileti dizisine gönderin
        if (snapshotRequsted.compareAndSet(true, false)) {
            /*
              * I420Frame, doku veya bellek içi arabellek olarak temsil edilebilir. yuvPlanes değil
              * çerçeve bellekte temsil edildiğinde ve yuvPlanes olduğunda null ve textureId sıfırdır
              * null textureId, çerçeve doku olarak temsil edildiğinde sıfır olmayan bir değerdir.
              */
            final Bitmap bitmap = i420Frame.yuvPlanes == null ?
                    captureBitmapFromTexture(i420Frame) :
                    captureBitmapFromYuvFrame(i420Frame);
            handler.post(() -> {
                // Görüntü görünümünün bit eşlemini güncelleyin
                imageView.setImageBitmap(bitmap);

                // Yerel belleği boşaltmak için çerçeveler oluşturulduktan sonra serbest bırakılmalıdır
                i420Frame.release();
            });
        } else {
            i420Frame.release();
    ...
    private Bitmap captureBitmapFromTexture(I420Frame i420Frame) {
        ...
        final float[] texMatrix = RendererCommon.multiplyMatrices(rotatedSamplingMatrix,
                layoutMatrix);
         /*
          * YuvConverter, etkin bir EGL içeriğine sahip bir iş parçacığı üzerinde başlatılmalıdır. Biliyoruz
          * bu renderFrame doğru render iş parçacığından çağrıldığından
          * Dönüştürücünün somutlaştırılmasını çerçeve gelene kadar erteleriz.
          */
        ...
        // Şimdi YUV verilerini uçaklara ayırmamız gerekiyor
        ...
        // YuvImage'ı jpeg olarak sıkıştır
        yuvImage.compressToJpeg(rect, 100, stream);

        // jpeg'i Bitmap'e dönüştür
        byte[] imageBytes = stream.toByteArray();

        // YUV Converter'ı serbest bırakın
